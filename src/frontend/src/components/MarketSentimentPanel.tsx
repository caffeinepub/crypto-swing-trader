...
export default function MarketSentimentPanel({
  ...
  const isExtremeSentiment =
    sentiment.fearGreedIndex < 20 || sentiment.fearGreedIndex > 80;
  ...
  return (
    <div className="grid gap-4 md:grid-cols-3 animate-fade-in">
      <Card
        className={`border-neon-cyan/30 bg-card ${isExtremeSentiment ? "glow-pulse" : "glow-ambient"}`}
      >
        <CardHeader className="pb-3">
          <CardTitle className="text-sm font-medium font-heading text-neon-cyan">
            Market Sentiment
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <Badge
                variant="outline"
                className={`${getSentimentColor()} border-current ${getSentimentGlow()} font-heading text-base px-3 py-1`}
              >
            ...
            <div className="space-y-2">
              ...
              <div className="text-center text-sm font-medium font-mono text-neon-cyan glow-text">
                {Math.round(sentiment.fearGreedIndex)}/100
              </div>
            </div>
          </div>
        </CardContent>
        ...
        <CardContent>
          <div className="space-y-2">
            {sentiment.topGainers.slice(0, 3).map((crypto) => (
              <div
                key={crypto.id}
                className="flex items-center justify-between text-sm p-2 rounded border border-neon-green/20 bg-neon-green/5 transition-all duration-300 hover:glow-hover"
              >
                <span className="font-medium truncate font-heading">
        ...
        <CardContent>
          <div className="space-y-2">
            {sentiment.topLosers.slice(0, 3).map((crypto) => (
              <div
                key={crypto.id}
                className="flex items-center justify-between text-sm p-2 rounded border border-neon-red/20 bg-neon-red/5 transition-all duration-300 hover:glow-hover"
              >
                <span className="font-medium truncate font-heading">
                  {crypto.symbol.toUpperCase()}
                </span>
