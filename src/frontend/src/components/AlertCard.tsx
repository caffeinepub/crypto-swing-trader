...
export default function AlertCard({ alert, cryptos }: AlertCardProps) {
  ...
  const cryptoSymbol =
    crypto?.symbol.toUpperCase() || alert.crypto.toUpperCase();
  const cryptoImage = crypto?.image;

  const signalType =
    alert.signalType === SignalType.buy
      ? "buy"
      : alert.signalType === SignalType.sell
        ? "sell"
        : "hold";
  ...
  const getTriggerReasonText = () => {
    ...
    if ("trendFollowing" in reason)
      return reason.trendFollowing ? "Trend Following" : "Trend Reversal";
  ...
  return (
    <Card
      className={`border ${getSignalBorderColor()} bg-card/80 ${getSignalGlow()} transition-all duration-300 hover:scale-[1.02] animate-fade-in relative overflow-hidden`}
    >
      ...
      <CardContent className="p-4 space-y-3 relative z-10">
        <div className="flex items-start justify-between gap-3">
          ...
          <Badge
            variant={variant}
            className={`flex-shrink-0 gap-1 font-heading ${getSignalTextColor()} glow-hover`}
          >
        ...
        <div className="space-y-2 text-sm">
          ...
          <div className="flex justify-between items-center">
            ...
            <span
              className={`font-bold font-mono ${Number(alert.confidence) >= 70 ? "text-neon-green" : Number(alert.confidence) >= 50 ? "text-yellow-400" : "text-neon-red"} glow-text`}
            >
